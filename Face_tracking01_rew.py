...
vs = VideoStream(src= 0, #'/dev/video2' , # was 0 voor webcam
...
def move_to_face(list_of_facepos,robot_pos):
    ...
    print(xyz_coords)
    # print("transform")
    #print(position_vec_coords)

    oriented_xyz = origin * position_vec_coords

    # x,y,z
    # this Z = 0 is actually z-400 (is 40 cm) .. that is weird
    #because of this offset i think movement is accelerated
    robot.movej(pose=[ 0.0, -0.6609,  0,    1.5694,  0.0117,  0.0147],a = ACCELERATION, v=VELOCITY)
    #robot.movej(pose=[ oriented_xyz.p[0], -0.6609,  oriented_xyz.p[2],    1.5694,  0.0117,  0.0147],a = ACCELERATION, v=VELOCITY)
    coordinates=[ oriented_xyz.p[0], -0.6609,  oriented_xyz.p[2],    1.5694,  0.0117,  0.0147]
    
    
    #coordinates=[ 0.0, -0.6609,  0.6255,    1.5694,  0.0117,  0.0147]
    #test = m3d.PoseVector(coordinates)
    #<PoseVec: P(0.000, -0.661, 0.625) RV(1.569, 0.012, 0.015)
...
try:
    ...
    while True:
        ...
        show_frame(new_frame) # shows frame with bounding box
        if len(face_positions) > 0: # if faces are found then move.
